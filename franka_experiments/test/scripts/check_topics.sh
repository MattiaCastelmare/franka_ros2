...
# Detect namespace from running joint_states topic
_detect_namespace() {
    local ns
    ns=$(ros2 topic list 2>/dev/null | grep '/joint_states' | head -1 | sed 's|/joint_states||' || echo "")
    echo "${ns}"
}
...
if [ -z "$PIPELINE" ]; then
    echo "Usage: $0 {velocity|acceleration|torque|oscbf|oscbf_obstacle}"
    exit 1
fi
...
_check_topic_active "${NS}/joint_states" 100.0 "joint_states rate"
...
case "$PIPELINE" in
  ...
  acceleration)
    ...
    _check_topic_exists "${NS}/torque_cmd" "torque_cmd (filter output)"
    ...
    _info "         cbf_safety_filter (HOCBF-QP) → /NS/torque_cmd"
  ...
  *)
    ...
    echo "Valid: velocity | acceleration | torque | oscbf | oscbf_obstacle"
    exit 1
    ;;
esac
