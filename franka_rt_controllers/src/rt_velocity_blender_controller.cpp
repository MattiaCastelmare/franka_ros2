...
int64_t RtVelocityExecutorController::steadyNowNs() {
...
std::array<double, RtVelocityExecutorController::kNumJoints>
RtVelocityExecutorController::interpolateInput(
...
RtVelocityExecutorController::command_interface_configuration() const {
...
controller_interface::InterfaceConfiguration
RtVelocityExecutorController::state_interface_configuration() const {
...
controller_interface::return_type RtVelocityExecutorController::update(
    const rclcpp::Time& /*time*/,
    const rclcpp::Duration& period) {

  const double dt = period.seconds();

  // ── 1. Read latest command from realtime buffer (lock-free RT side) ──
  const auto command = *command_buf_.readFromRT();

  // ── 2. Optional linear interpolation (reduces sample-and-hold steps) ─
  ...
  if (enable_interpolation_) {
    ...
    qdot_cmd = interpolateInput(command, command_interp_, now_ns);
  } else {
    qdot_cmd = command.qdot;
  }

  // ── 3. Rate limiter  (optional – only when max_accel_ > 0) ──────────
  ...
  // ── 4. Final clamp  (single, authoritative – only when qdot_max_ > 0)
  if (qdot_max_ > 0.0) {
    for (size_t i = 0; i < kNumJoints; ++i) {
  ...
  // ── 5. Write to hardware command interfaces ──────────────────────────
  for (size_t i = 0; i < kNumJoints; ++i) {
    command_interfaces_[i].set_value(qdot_cmd[i]);
  }

  // ── 6. Store for next cycle's rate limiter ───────────────────────────
  prev_cmd_ = qdot_cmd;
...
CallbackReturn RtVelocityExecutorController::on_init() {
  try {
    ...
    auto_declare<std::string>("command_topic", "qdot_cmd");
...
CallbackReturn RtVelocityExecutorController::on_configure(
  ...
  timeout_ramp_s_      = get_node()->get_parameter("timeout_ramp_s").as_double();
  ...
  command_buf_.writeFromNonRT(zero_input);

  // ── Create subscriber (non-RT callback, BestEffort depth=1) ───────
  ...
  // ── Summary log ───────────────────────────────────────────────────
  RCLCPP_INFO(logger, "╔══ RtVelocityExecutorController configured ══╗");
  RCLCPP_INFO(logger, "  arm_id       : %s", arm_id_.c_str());
  for (size_t i = 0; i < kNumJoints; ++i) {
    RCLCPP_INFO(logger, "  joint[%zu]     : %s", i, joint_names_[i].c_str());
  }
  RCLCPP_INFO(logger, "  command_topic: %s", command_topic_.c_str());
...
CallbackReturn RtVelocityExecutorController::on_activate(
    const rclcpp_lifecycle::State& /*previous_state*/) {
  ...
  // Reset buffer and interpolation state
  VelocityInput zero_input{};
  command_buf_.writeFromNonRT(zero_input);
  command_interp_ = InterpState{};

  RCLCPP_INFO(get_node()->get_logger(),
              "RtVelocityExecutorController ACTIVATED – writing velocity "
...
CallbackReturn RtVelocityExecutorController::on_deactivate(
    const rclcpp_lifecycle::State& /*previous_state*/) {
  ...
  RCLCPP_INFO(get_node()->get_logger(),
              "RtVelocityExecutorController DEACTIVATED → zero velocity.");

  return CallbackReturn::SUCCESS;
}

// ═══════════════════════════════════════════════════════════════════════════
//  Subscriber callback  (non-RT thread – may allocate, may log)
// ═══════════════════════════════════════════════════════════════════════════

void RtVelocityExecutorController::commandCb(
    const std_msgs::msg::Float64MultiArray::SharedPtr msg) {

  if (msg->data.size() != kNumJoints) {
    RCLCPP_WARN_THROTTLE(
        get_node()->get_logger(), *get_node()->get_clock(), 1000,
        "command_topic: expected %zu values, got %zu – IGNORING",
...
//  Plugin registration
// ═══════════════════════════════════════════════════════════════════════════
PLUGINLIB_EXPORT_CLASS(franka_rt_controllers::RtVelocityExecutorController,
